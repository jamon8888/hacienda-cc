---
description: >
  Use when the user runs /hm:build. Scaffolds plugin skeleton from evals
...
**If `hm.json` exists with `use_cases` and `evals`:**
1. Scaffold plugin skeleton from evals
2. Run validation
3. Run baseline evaluation
4. Initialize TSV log

**If `hm.json` missing or has no evals:**
...
Summary:
1. Check for `hm.json` in working directory
